...
<div class="flex items-center justify-start gap-3">
  <div class="rounded-lg flex-center">
    <img src=${data?.imageUrl || data?.icon_url || data?.logoUrl || "https://app.uncx.network/img/no-icon.0e5c6c82.png"} alt="icon" class="w-[80px] h-[100px] object-contain">
  </div>
  <div>
    <h1 class="cont text-lg">${data?.tokenName || data?.s_token?.name || data?.tokenOfSaleName}</h1>
    <p class="nunito text-md">${data?.tokenSymbol || data?.s_token?.symbol || data?.tokenOfSaleSymbol}</p>
  </div>
</div>
<div class="w-full">
  <div class="flex gap-3 text-xl monospace">
    ...
    <span class="blue-text capitalize">
      ${data?.platform == 'pink' ? "Pinksale" : data?.s_token ? "UNCX" : data?.platform || "DXSale" }
    </span>
  </div>
  <div>
    <!-- CURRENCY -->
    ...
    <div class="flex items-start justify-between w-[90%] gap-2">
      <div class="nunito">
        ...
        <h3 class="text-2xl">${formatNumber(parseFloat(data?.softCap || data?.saleSoftCap)) || "N/A"}</h3>
      </div>
      <div class="nunito">
        <p class="opacity-80 monospace text-lg">HARD CAP</p>
        <h3 class="text-2xl">${formatNumber(parseFloat(data?.hardCap || data?.saleHardCap)) || "N/A"}</h3>
      </div>
    </div>
    <!-- PROJECT LINKS -->
    <div style="position: relative" class="mt-8 mb-3 pb-2">
        ...
        <p class="truncate-text text-sm text-left opacity-80 monts mr-5">
          ${data?.socials?.description || formatDesc(data?.descriptionText) || ""}
        </p>
        <p style="position: absolute" class="!absolue bottom-0 opacity-40 right-5 text-xs">
          ${(data?.socials?.description || data?.descriptionText) ? 'see more...' : ""}.
        </p>
    </div>
    <div class="flex justify-start">
      ${
        (data?.link || data?.website_url || data?.websiteUrl) ?
      `<a href=${data?.link || data?.website_url || data?.websiteUrl} class="text-lg w-[40%] mt-5 nunito blue-grad py-2 rounded-lg turner text-center">
